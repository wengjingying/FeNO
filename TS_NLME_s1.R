...
TS_NLME_S1<-function(dat,tol1,tol2){
  mydat <- subset(dat,select=c(yr_id,logeno,flow,id)) #,id%in%unique(dat$id)[1:50]
  ...
  idvisitmat <- matrix(as.integer(unlist(strsplit(rownames(coef(fitnlsList)),"/"))),byrow=T,ncol=1)
  for(i in 1:3){
  ...
  fit1.nlme <- nlme(logeno~SSnonLinLogPlogJwoGrad(flow,logCawNO,CaNO,logDawNO),
                    ...
                    start=list( fixed=c(1.82,4.12,2.63),
                                random=list(id=idREstart) # only speed up cvg by ~5% overall
...
TS_NLME_S2<-function(dat,fit,X){
  mydat <- subset(dat,select=c(yr_id,logeno,flow,id)) #,id%in%unique(dat$id)[1:50]
  nlmeFit_yrinid_logCawlogDawCa <- fit
  cf <- coef(fit)
  startfix <- unlist(lapply(cf, median, na.rm = TRUE))
  idvisitmat <- matrix(as.integer(unlist(strsplit(rownames(coef(fit)),"/"))),byrow=T,ncol=1)
  ...
  nlmeFit_yrinid_logCawlogDawCa_CaNO_est      <- idREyr[rownames(cf),"CaNO"] + ranef(fit)[rownames(cf),"CaNO"] + fixef(fit)["CaNO"]
  nlmeFit_yrinid_logCawlogDawCa_logCawNO_est  <- idREyr[rownames(cf),"logCawNO"] + ranef(fit)[rownames(cf),"logCawNO"] + fixef(fit)["logCawNO"]
  nlmeFit_yrinid_logCawlogDawCa_logDawNO_est  <- idREyr[rownames(cf),"logDawNO"] + ranef(fit)[rownames(cf),"logDawNO"] + fixef(fit)["logDawNO"]
  ...
  nlmeFit_est<-nlmeFit_est[order(nlmeFit_est$id),]
