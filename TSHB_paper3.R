...
#model: idlist Nvisit flowarray logeno 
Jags_TS_CHS <- function()  {
  for( i in 1 : Nid) {
...
#idlist=idlist
#yr_perID=yr_perID
data_TS_CHS_8 <- list(Nid=length(unique(Enodata8$id)),offset=offset8,
                         logeno=Enodata8$logeno,flow=Enodata8$flow,
                         beta0_prior=beta0_prior
)


param.TS_CHS_8 <-c("alpha_Ca","alpha_logCaw","alpha_logDaw",
                      "Ca","logCaw","logDaw",
...
time<-Sys.time()
LongiX_sim_2<- jags(data=data_TS_CHS_8, parameters.to.save=param.TS_CHS_8, inits=inits_TS,
                    model.file=Jags_TS_CHS, n.chains=3, n.iter=N.iterT, n.burnin=N.burn,n.thin=N.thinM) 
...
while(N_updateT<Max_update){
  if(max(LongiX_sim_2$BUGSoutput$summary[rownames(LongiX_sim_2$BUGSoutput$summary) %in% param.Rhat,"Rhat" ])<rhat){
    LongiX_sim_2<-update(LongiX_sim_2,n.iter=n.final,n.thin=20)
    N_final=N_final+1
    print("final")
    print(N_final)
    print(LongiX_sim_2$BUGSoutput$summary[rownames(LongiX_sim_2$BUGSoutput$summary) %in% param.Rhat,"Rhat" ])
    if(max(LongiX_sim_2$BUGSoutput$summary[rownames(LongiX_sim_2$BUGSoutput$summary) %in% param.Rhat,"Rhat" ])<rhat){
      break
    }
  }
...
save(result,LongiX_sim_2,file="new_CHS_JAGS_TS_paper3_y8.Rdata")


save(result,file="new_D_CHS_JAGS_TS_paper3_y8.Rdata")
